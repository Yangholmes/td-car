...
var b=[];console.log(a);a.reservation.map(function(a){b.push(a["schedule-start"].replace(/^\d{4}-/,"")+" ~ "+a["schedule-end"].replace(/^\d{4}-/,""))});a.suspend.map(function(a){b.push(a["schedule-start"].replace(/^\d{4}-/,"")+" ~ "+a["schedule-end"].replace(/^\d{4}-/,"")+"\u672a\u5f52\u8fd8")});d.eq(0).attr("id","td-car-item-"+a.carid);d.eq(1).attr("id","td-car-detail-"+a.carid);d.find("img").attr("src",a.imageSrc);d.find(".td-form-comb-img-text-item-text").html(a.model);d.find(".td-car-info-plate-number").html(a.plateNumber);
d.find(".td-car-info-seating").html(a.seating);d.find(".td-car-reservation-time").html(0===b.length?"\u8fd1\u65e5\u65e0\u7ea6\u8f66\u8bb0\u5f55":b.join("<br>"));d.appendTo(c)})},tdFormController=function(){function a(a){a?($(".transparent-mask").addClass("popup").show(),this.addClass("popup").fadeIn(200),$("body").addClass("fixed")):(this.addClass("popup").fadeOut(200),$(".transparent-mask").addClass("popup").hide(),$("body").removeClass("fixed"))}function c(a){e||(a.stopPropagation(),$(a.currentTarget).remove())}
function b(a,h,b,k,c){dd.biz.contact.choose({startWithDepartmentId:a,multiple:h,users:b,disabledUsers:k,corpId:_config.corpId,limitTips:"\u6311\u592a\u591a\u5566\uff01",isNeedSearch:!0,title:"\u6311\u4e2a\u4eba\u5457~",local:"false",onSuccess:function(a){c(a)},onFail:function(a){$.tdAlert("\u975e\u5e38\u62b1\u6b49\uff01\n\u60a8\u7684\u901a\u4fe1\u5f55\u6253\u4e0d\u5f00\u3002\u3002\u3002")}})}function d(a){var f=$(a.currentTarget).children("input"),b=$(a.currentTarget).find("ul li input");a=[];for(var b=
$jscomp.makeIterator(b.toArray()),c=b.next();!c.done;c=b.next())value=c.value,a.push(JSON.parse(value.value));f.val(JSON.stringify(a))}var e=!1;$("body").on("touchstart",function(a){e=!1});$("body").on("touchmove",function(a){e=!0});$(".td-form-datetime-picker input").on("focus",function(a){e||(a.stopPropagation(),$(a.currentTarget).blur(),dd.biz.util.datetimepicker({format:"yyyy-MM-dd HH:mm",value:"",onSuccess:function(f){$(a.currentTarget).val(f.value).blur()},onFail:function(){$(a.currentTarget).blur()}}))});
$(".td-form-comb").on("touchend",function(f){e||a.call($(f.currentTarget).find("ul"),!0)});$(".td-form-comb ul li").on("touchend",function(f){if(!e){f.stopPropagation();var b=$(f.currentTarget).parent("ul").prev(".td-form-comb-selected").find(".td-form-comb-item"),c=$(f.currentTarget).html(),d=f.currentTarget.id;b.html(c).attr("id",d).find(".td-form-field-detail").removeClass("fa fa-hand-pointer-o");$(f.currentTarget).parent("ul").prevAll("input").val(d);a.call($(f.currentTarget).parent("ul"))}});
$(".td-form-field-detail").on("touchend",function(a){e||(a.stopPropagation(),a=$(a.currentTarget).parent("li").next(".td-form-comb-img-text-item-detial"),$(".td-form-comb-img-text-item-detial").slideUp(),"none"==a.css("display")&&a.slideDown())});$(".td-form-easy-picker").on("touchend",function(a){e||(a.stopPropagation(),b(0,!1,[_user.userid],[],function(b){$(a.currentTarget).find("output").val(b[0].name);$(a.currentTarget).find("input").val(JSON.stringify(b[0]))}))});$(".td-form-approver-picker").on("upload",
d).find("ul div.td-form-approver-picker-add").on("touchend",function(a){e||(a.stopPropagation(),b(0,!1,[],[_user.userid],function(b){if(b[0].emplId==_user.userid)return $.tdAlert("\u4e0d\u80fd\u591f\u81ea\u5df1\u5ba1\u6279\u81ea\u5df1\u54e6~"),!1;var c=$(a.currentTarget).parents(".td-form-approver-picker").find("ul li#admin"),d=$('<li class="td-form-approver-picker-item"><input type="hidden" class="td-form-input-hidden"><div class="td-form-approver-picker-item-avatar fa fa-arrow-right"><img src='+
