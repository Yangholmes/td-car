...
var _car={},tdCarInit=function(){$.ajax({url:"server/reservation/application-init.php",method:"GET",dataType:"json",data:"keyword=yangholmes",error:function(a,d,c){console.log(a)},success:function(a,d,c){0==a.error?(_car=a.records,tdFormView(),tdFormData(),tdFormController()):alert("\u8f66\u8f86\u521d\u59cb\u5316\u5931\u8d25\n\u8bf7\u91cd\u8bd5")}})},tdFormView=function(){/[Aa]ndroid/.test(navigator.userAgent)&&($("div.td-form-button").css("position","fixed").css("bottom","0"),$("div.td-form-field:nth-last-child(2)").css("margin-bottom",
...
onFail:function(a){alert("\u4f60\u7684\u901a\u4fe1\u5f55\u6253\u4e0d\u5f00\u3002\u3002\u3002")}}))});$(".td-form-approver-picker").on("upload",c).find("ul div.td-form-approver-picker-add").on("touchend",function(a){b||(a.stopPropagation(),dd.biz.contact.choose({startWithDepartmentId:0,multiple:!1,users:[],disabledUsers:[_user.userid],corpId:_config.corpId,limitTips:"\u6311\u592a\u591a\u5566\uff01",isNeedSearch:!0,title:"\u6311\u4e2a\u4eba\u5457~",local:"false",onSuccess:function(b){console.log(b);
var c=$(a.currentTarget).parents(".td-form-approver-picker").find("ul li#admin"),e=$('<li class="td-form-approver-picker-item"><input type="hidden" class="td-form-input-hidden"><div class="td-form-approver-picker-item-avatar fa fa-arrow-right"><img src='+b[0].avatar+'></div><div class="td-form-approver-picker-item-name">'+b[0].name+"</div></li>");e.on("touchend",f).find("input").val(JSON.stringify(b[0]));c.before(e)},onFail:function(a){alert("\u4f60\u7684\u901a\u4fe1\u5f55\u6253\u4e0d\u5f00\u3002\u3002\u3002")}}))});
...
e=$('<li class="td-form-cc-picker-item"><input type="hidden" class="td-form-input-hidden"><div class="td-form-cc-picker-item-avatar"><img src='+b[0].avatar+'></div><div class="td-form-cc-picker-item-name">'+b[0].name+"</div></li>");e.on("touchend",d).find("input").val(JSON.stringify(b[0]));c.before(e)},onFail:function(a){alert("\u4f60\u7684\u901a\u4fe1\u5f55\u6253\u4e0d\u5f00\u3002\u3002\u3002")}}))});$(".td-form-button .td-button").on("touchstart",function(a){$(".td-form-approver-picker").trigger("upload");
$(".td-form-cc-picker").trigger("upload");$("input#applicant").val(JSON.stringify({avatar:_user.avatar,emplId:_user.userid,name:_user.name}));var b=$("form");a=new FormData(b[0]);for(var b=b.find("*[required]"),b=$jscomp.makeIterator(b),c=b.next();!c.done;c=b.next())if(field=c.value,!field.value)return alert($(field).prev("label").css("color","red").animate({opacity:0},500,function(){$(this).css({opacity:"1",color:"black"})}).html()+"\u5fc5\u586b\u54e6~"),!1;$.ajax({url:"server/reservation/apply.php",
type:"POST",data:a,dataType:"json",processData:!1,contentType:!1,cache:!1,beforeSend:function(){dd.device.notification.showPreloader({text:"\u6b63\u5728\u4f7f\u52b2\u63d0\u4ea4\u7533\u8bf7...",showIcon:!0,onSuccess:function(a){},onFail:function(a){}})},success:function(a,b,c){dd.device.notification.hidePreloader({onSuccess:function(a){},onFail:function(a){}});0==a.error?(localStorage.setItem(a.records.resid,JSON.stringify(a.records)),window.location.href="page/approval.html?resid="+a.records.resid):
alert(a.errorMsg)},error:function(a,b,c){dd.device.notification.hidePreloader({onSuccess:function(a){},onFail:function(a){}});alert("\u7533\u8bf7\u5931\u8d25\uff01T.T")}})});$(".transparent-mask").on("touchend",function(a){$(".popup").fadeOut(200);$("body").removeClass("fixed")});$("body").on("touchmove",function(a){console.log(a)})};
