 ...
 * td-alert
 */
$.tdAlert = function(msg, callback, title){
 ...
 * td-confirm
 */
$.tdConfirm = function(msg, callback, title) {
	var tdConfirm = new MessageBox(1, title?title:'提示', msg, '', callback);
	return true;
};

$.tdPrompt = function(msg, callback, title, placeholder) {
	var tdPrompt = new MessageBox(2, title?title:'输入', msg, placeholder, callback);
	return true;
};
/**
 * Message Class
 */
function MessageBox(type, title, message, placeholder, callback){
	this.type = type;
	this.setTitleContext( title );
	this.setContextContext( message );
	this.setContextArea( placeholder );
...
MessageBox.prototype = {
	...
	_renderMessage () {
		...
		if( this.type == 2 )
			this._renderContextArea();
		this.$message =
			$(this.messageHTML).css(this.messageStyle)
			.append(this.$title).append(this.$context).append(this.$contextarea);
	},
	 ...
	 */
	$contextarea: {},
	contextareaHTML: `<textarea class="message-box-textarea" rows="4"></textarea>`,
	contextareaStyle: {'width':'100%', 'border-radius':'4px'},
	areaPlaceholder: '',
	setContextArea (msg) { this.areaPlaceholder = msg ?  msg : this.areaPlaceholder },
	_renderContextArea () {
		this.$contextarea = $(this.contextareaHTML).css(this.contextareaStyle).attr('placeholder',this.areaPlaceholder);
	},
	...
	_renderButton () {
		...
		this.$button =
			...
			.append(this.$cancel).append(this.$confirm).append(this.$prompt)
			.on('touchstart', function(e){
				$(e.target).css({
		    ...
		    self._destory().callback(/message-box-button-confirm/.test(e.target.outerHTML), self.type==2?self.$contextarea[0].value:null);
		  });
	},
	...
	cancelStyle: { 'text-align': 'center', width: '100%', 'line-height': '2em', padding: '.2em', 'border-right': '1px solid rgba(150, 150, 150, 0.2)' },
	cancelText: '不',
	_renderCancel () {
		this.$cancel =
	...
	confirmStyle: { 'text-align': 'center', width: '100%', 'line-height': '2em', padding: '.2em' },
	confirmText: '嗯',
	_renderConfirm () {
		this.$confirm =
	 ...
	 */
	$prompt: {},
	promptHTML: `<div class="message-box-button-confirm"></div>`,
	promptStyle: { 'text-align': 'center', width: '100%', 'line-height': '2em', padding: '.2em' },
	promptText: '提交',
	_renderPrompt () {
		this.$prompt =
			$(this.promptHTML).css(this.promptStyle).html(this.promptText);
	},
	...
	/**
	 * [destory mask and frame]
	 * @return {[type]} [description]
	 */
	_destory () {
		this.$mask.fadeOut(100, () => { this.$mask.remove() });
		this.$frame.fadeOut(100, () => { this.$frame.remove() });
		return this;
	}
};
